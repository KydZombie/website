...
export function addHovering(element: HTMLElement) {
  element.addEventListener("mouseenter", () => setStatus(true));
  element.addEventListener("mouseleave", () => setStatus(false));
}

export function addHoveringToAll(elements: Array<HTMLElement>) {
  elements.forEach(element => {
    addHovering(element);
  });
}

export function addHoveringToTag(filter: string) {
